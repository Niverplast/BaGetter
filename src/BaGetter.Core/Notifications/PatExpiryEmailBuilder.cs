...
public class PatExpiryEmailBuilder : IPatExpiryEmailBuilder
{
    ...
    public EmailMessage Build(PersonalAccessToken token, int daysUntilExpiry)
    {
        ...
        var callToAction = tokensLink is null
            ...
            : $"""<a href="{System.Net.WebUtility.HtmlEncode(tokensLink)}">Create a replacement token ({_options.WebBaseUrl})</a> before then.""";
        ...
        var body =
            $"""
            <p>Your Niverplast Package Feed personal access token <strong>{tokenNameHtml}</strong> (prefix <code>{tokenPrefixHtml}</code>) expires {whenText}, on <strong>{token.ExpiresAtUtc:yyyy-MM-dd HH:mm} UTC</strong>.</p>
